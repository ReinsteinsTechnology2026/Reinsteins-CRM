...
(async () => {
    ...
    // Phase 16A discovery: PostgreSQL's table_schema means the SCHEMA
    // ('public'), not the database name -- the old MySQL-style
    // `TABLE_SCHEMA = ?` [DB_NAME] query silently matched zero rows
    // against Postgres. Also excludes the platform tables by name,
    // since production's DB_NAME and PLATFORM_DB_NAME are the SAME
    // database (see schemas/platformSchema.postgresql.sql) -- without
    // this exclusion, a bare table_schema='public' count would grow
    // every time a platform-level table is added, even though nothing
    // about the Reinsteins TENANT schema changed.
    ...
    check("Reinsteins tenant DB unchanged (37 tables, 17 users)", Number(tbl) === 37 && Number(users) === 17, `tables=${tbl} users=${users}`);
