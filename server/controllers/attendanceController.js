...
const getMonthlyAttendanceHistory = async (
  req,
  res
) => {
  try {
    ...
    const firstLoginMonth =
      `${firstLoginDate.getFullYear()}-${String(
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "You cannot view attendance before your first login month.",
      });
    }
    ...
    // ========================================
    // FIRST LOGIN DATE KEY
    // ========================================

    const firstLoginDateKey =
      firstLoginDate
        .toISOString()
        .slice(0, 10);
    ...
    console.error(
      "Monthly Attendance History Error:",
      error
    );
    ...
    return res.status(500).json({
      ...
      message:
        "Unable to load monthly attendance history",
    });
  }
};
