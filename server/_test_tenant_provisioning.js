...
(async () => {
    ...
    const tblCountResult = await sourceConn.query(
        `SELECT COUNT(*) AS "tblCountBefore"
         FROM information_schema.tables
         WHERE table_schema = 'public'
         AND table_type = 'BASE TABLE'`
    );

    const tblCountBefore = Number(tblCountResult.rows[0].tblCountBefore);

    const userCountResult = await sourceConn.query(
        `SELECT COUNT(*) AS "userCountBefore" FROM users`
    );

    const userCountBefore = Number(userCountResult.rows[0].userCountBefore);
...
const sourceConnAfter = new Client({
    host: process.env.DB_HOST,
    port: process.env.DB_PORT,
    user: process.env.DB_USER,
    password: process.env.DB_PASSWORD,
...
await sourceConnAfter.connect();

const tblCountAfterResult = await sourceConnAfter.query(
    `SELECT COUNT(*) AS "tblCountAfter"
     FROM information_schema.tables
     WHERE table_schema = 'public'
     AND table_type = 'BASE TABLE'`
);

const tblCountAfter = Number(tblCountAfterResult.rows[0].tblCountAfter);

const userCountAfterResult = await sourceConnAfter.query(
    `SELECT COUNT(*) AS "userCountAfter" FROM users`
);

const userCountAfter = Number(userCountAfterResult.rows[0].userCountAfter);

check(
    "reinsteins_workhub table count unchanged",
    tblCountAfter === tblCountBefore,
    `before=${tblCountBefore} after=${tblCountAfter}`
);

check(
    "reinsteins_workhub user count unchanged",
    userCountAfter === userCountBefore,
    `before=${userCountBefore} after=${userCountAfter}`
);

await sourceConnAfter.end();
